...
define("REQUEST_COMMENT",'SELECT	c.comment_no,
									...
									c.time,
									u.nick_name
									FROM dynamic_webtoon_comment c, _user u 
									WHERE c.post_no = '.$post_no.' AND
									c.user_no = u.id_no AND
									c.state = 0
									ORDER BY c.comment_no DESC;', FALSE);

define("REQUSET_REGISTER_COMMENT","INSERT INTO dynamic_webtoon_comment VALUES(NULL, ",FALSE);									
define("REQUEST_LATEST_COMMENT_NO", "SELECT max(comment_no) AS cmt_cnt from dynamic_webtoon_comment;" ,FALSE); 
define("REQUEST_DELETE_COMMENT", "UPDATE dynamic_webtoon_comment SET state = -1 WHERE comment_no = ",FALSE);
define("REQUEST_UPDATE_COMMENT", "UPDATE dynamic_webtoon_comment SET comment_ = ",FALSE);
